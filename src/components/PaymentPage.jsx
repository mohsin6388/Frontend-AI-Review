...
const PaymentPage = ({ user }) => {
  ...
  const handlePayment = async (planId, cycle, couponCode) => {
    ...
    try {
      ...
      const { data } = await api.post(
        "/payment/create-order",
        { user_id: user.id, plan_name: backendPlanName, coupon_code: couponCode || null },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const handleApplyCoupon = () => {
    ...
    // TEMP: fake API delay — real backend ready hote hi isko api.post("/payment/validate-coupon", ...) se replace karna
    setTimeout(() => {
      const coupon = MOCK_COUPONS[codeEntered];

      if (coupon) {
        const discountAmount =
          coupon.discount_type === "percent"
            ? Math.round((modalPrice * coupon.discount_value) / 100)
        ...
        >
          ...
          >
            ...
            <div
              ...
              style={{ margin: "16px 0", width: "100%", boxSizing: "border-box" }}
            >
